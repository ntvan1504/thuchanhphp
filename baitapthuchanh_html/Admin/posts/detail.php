...
if ($router->getPOST("submit") && $router->getPOST("name") && $router->getPOST("content") && $router->getPOST("category")
) {
//    $params = [
//        ":name" => $router->getPOST("name"),
//        ":cont" => $router->getPOST("content"),
//        ":desc" => $router->getPOST("description"),
//        ":cate" => $router->getPOST("category"),
//    ];
    $name = $router->getPOST("name");
    $cont = $router->getPOST("content");
    $desc = $router->getPOST("description");
...
//        $params[":id"]=$id;
        $result = $post->buildQueryParams([
                    "value" => "name='$name', content='$cont', description='$desc', cate_id='$cate'",
                    "where" => "id=" . $router->getGET("id"),
//            "params"=>$params
                ])->update(); //nếu có id thì tiến hành update
    } else {
        $result = $post->buildQueryParams([
                    "field" => "(cate_id, name, description, content, created_by, created_time) VALUES (?,?,?,?,?,now())", //created_time là thời gian hiện tại. //trong ngoặc có bao nhiêu phần tử thì phải có bấy nhiêu dấu hỏi
                    "value" => [$cate, $name, $desc, $cont, $user->getId()] //created_by là userId lấy được bằng hàm getId
...
<html lang="en">
    ...
    <head>
        ...
        <title>Admin Posts - Đồ ăn thức uống</title>
    ...
    <body>
        ...
        <div id="wrapper">
            ...
            <div id="page-wrapper">
                <div class="row">
                    <div class="col-lg-12">
                        <h1 class="page-header"><?= !$id ? "Tạo mới " : "Xem nội dung " ?> bài viết <?= $postDetail ["name"] ?></h1>
                    </div>
                    <!-- /.col-lg-12 -->
                </div>
                <!-- /.row -->
                            ...
                            <div class="panel-body">
                                <div class="row">
                                    <div class="col-lg-6" class="show-data">
                                        <form role="form" action="<?php echo $router->createUrl('posts/detail', ["id" => $postDetail["id"]]) ?>" method="POST">
                                            <div class="form-group">
                                                <label>Title</label>
                                                <input class="form-control" placeholder="Tiêu đề bài viết" type="text" name="name" value="<?= $postDetail["name"] ?>">
                                            </div>
                                            <div class="form-group">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <textarea class="form-control" placeholder="Giới thiệu nội dung" rows="3" name="description"><?= $postDetail["description"] ?></textarea>
                                            </div>
                                            <div class="form-group">
                                                <label>Content</label>
                                                <textarea class="form-control" placeholder="Nội dung chi tiết" rows="3" name="description" name="content"><?= $postDetail["content"] ?></textarea>
                                            </div>

<!--                                            <button type="submit" class="btn btn-info"><input type="submit" name="submit" value="Post"></button>
<button type="reset" class="btn btn-info"><input onclick="window.location.href = '<?= $router->createUrl("posts/index") ?>'" type="button" value="Cancel"></button>-->
                                            <input class="btn btn-info" type="submit" name="submit" value="Post">
                                            <input class="btn btn-info" onclick="window.location.href = '<?= $router->createUrl("posts/index") ?>'" type="button" value="Cancel">
                                        </form>
                                    </div>
                                    <!-- /.col-lg-6 (nested) -->
